...
import { setUser, setFilter, authActions } from "../../redux/auth/actions";
...
const Header = () => {
  ...
  const [filter, setFilterState] = useState("available");
  ...
  async function handleChangeAvailability(e) {
    const availability = e.target.value;
    setFilterState(availability);
    dispatch({ type: authActions.SETFILTER, filter: availability });
    setOpenAvatar(false);
  ...
  const menuStyling = {
    menuOpacity: `${openAvatar ? "opacity-10" : "opacity-0"}`,
    menuVisibility: `${openAvatar ? "block" : "hidden"}`,
    menuTransition: `${openAvatar ? "translate-x-0" : "translate-x-[105%]"}`,
  };
  ...
  return (
    <div className="p-2 bg-[#EBF3FC] border-b border-[#a0a6b124] flex justify-between items-center w-full right-4 z-50">
      ...
      <div className="relative flex gap-10">
        ...
        <img className="w-9 h-9 bg-[#aaa] rounded-full cursor-pointer object-cover" onClick={handleOpenAvatar} src={user.avatar} />
        {/* Menu */}
        <div
          ...
          open={openAvatar}>
          <div className="text-3xl mt-3 cursor-pointer	text-[#666] block w-[45px] py-0	px-3.5	ml-auto	md:hidden " onClick={handleClickAvatar}>
            &times;
          </div>
